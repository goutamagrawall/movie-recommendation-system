...
# FASTAPI APP
# =========================
# Shared HTTP client — initialized in lifespan, reuses TCP connections
http_client: Optional[httpx.AsyncClient] = None


@asynccontextmanager
async def lifespan(app):
    """Load pickle files at startup, create shared HTTP client."""
    global http_client
    load_pickles()
...
async def tmdb_get(path: str, params: Dict[str, Any]) -> Dict[str, Any]:
    """
    Safe TMDB GET using the shared http_client:
    - Network errors -> 502
...
def build_title_to_idx_map(indices: Any) -> Dict[str, int]:
    ...
    Logs a warning for duplicate titles (last one wins).
    """
    import sys

    if not hasattr(indices, "items"):
        raise RuntimeError(
            f"indices.pkl must be a dict or pandas Series (has .items()), "
    ...
    for k, v in indices.items():
        ...
        if key in title_to_idx:
            print(
                f"WARNING: duplicate title '{k}' in indices.pkl — "
                f"overwriting idx {title_to_idx[key]} with {int(v)}",
                file=sys.stderr,
            )
        title_to_idx[key] = int(v)
    return title_to_idx
...
def tfidf_recommend_titles(
    ...
    # cosine similarity between the query movie and all others
...
def load_pickles():
    ...
    # Load and validate df
    with open(DF_PATH, "rb") as f:
        df = pickle.load(f)
    if not isinstance(df, pd.DataFrame):
        raise RuntimeError(f"df.pkl must be a DataFrame, got {type(df).__name__}")
    if "title" not in df.columns:
        raise RuntimeError(
            f"df.pkl DataFrame must have a 'title' column. "
    ...
    if not hasattr(indices_obj, "items"):
        raise RuntimeError(
            f"indices.pkl must be dict or Series, got {type(indices_obj).__name__}"
        )

    # Load and validate TF-IDF matrix (usually scipy sparse)
    with open(TFIDF_MATRIX_PATH, "rb") as f:
        tfidf_matrix = pickle.load(f)
    if not hasattr(tfidf_matrix, "shape") or len(tfidf_matrix.shape) != 2:
        raise RuntimeError(
            f"tfidf_matrix.pkl must be a 2D matrix, got {type(tfidf_matrix).__name__}"
        )
    ...
    if len(df) != tfidf_matrix.shape[0]:
        raise RuntimeError(
            f"Dimension mismatch: df has {len(df)} rows but "
...
async def search_bundle(
    ...
    # Fetch all TMDB cards concurrently instead of sequentially (N+1 fix)
